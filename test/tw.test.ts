...
describe("tailwind", () => {
  ...
  it("copies the class value into the tw prop", () => {
    const result = html`<div class="m-4">Hello world</div>`;
    expect(result.props.children?.[0]?.props["tw"]).toEqual("m-4");
  });
  it("ignores non-tailwind classes", () => {
